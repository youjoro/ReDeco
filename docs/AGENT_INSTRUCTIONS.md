...
2 — Highest-risk security issues (must fix immediately)
    ...
    Secrets committed in repo
        Where: .replit contains VITE_SUPABASE_ANON_KEY, VITE_SUPABAY_URL, PIXABAY key.
        Risk: exposure, abuse, quota theft, unauthorized reads/writes.
        Immediate action: remove secrets from repo, rotate the keys, set secrets in the platform secret store.
        Result: keys no longer in source; rotated secrets stop ongoing abuse.
        Don’t: re-commit any secret, obfuscate secrets in repo, or paste them into docs.
    ...
    Public storage bucket for uploaded images
        ...
        Immediate action: make bucket private; issue signed URLs for client access; require RLS/metadata checks.
    ...
    Exposed third-party keys in client
        Where: Pixabay key used in client.
        Risk: quota theft and abusive usage.
        Fix: proxy calls through a backend (serverless function) that injects the key, enforces per-user rate limits, and logs usage.
...
4 — What the repo needs to be production-ready (prioritized) Emergency (0–24 hours)

    Remove secrets from repo and rotate keys.
    Make storage private and switch to signed URLs.
...
Short term (1–7 days)
    ...
    Harden Supabase RLS policies and audit all DB queries that rely on user id.
...
Mid term (1–4 weeks)
    ...
    Add monitoring/alerts (Sentry properly configured and tested).
...
5 — AI agent policy (how an automation should behave) This section is addressed to any AI agent that will read this file. Agents MUST follow these rules before performing changes.
...
Agent suggested workflow
    ...
    Phase 3 (human operation): rotate keys, change bucket privacy, configure platform secrets.
...
Example (.ai-agent/config.json) { "commit_threshold_units": 20, "per_task_cost": { "small": 1, "medium": 7, "large": 25 }, "require_human_for": [ "secrets", "key_rotation", "storage_privacy_changes", "prod_env_changes" ], "stop_file": ".ai-agent/AI_STOP" }
...
available = platform.getCredits() # must exist; else ask human config = repo.read('.ai-agent/config.json') or defaults if repo.exists(config.stop_file): exit("STOP file present") if available < config.commit_threshold_units: propose_pr("Insufficient credits; I can do X (small) now. Need Y more credits for full migration.") else: perform_small_fixes() # id bug, listener cleanup, tsconfig, lint updates create_branch_and_pr("migration/initial-ts-and-cleanup", changes) run_ci_and_report()

8 — Examples of specific fixes & results (copy/paste)

    Fix id bug — change this (src/App.jsx): BEFORE: id: local-${nanoid()}, src, label, AFTER: id: local-${nanoid()}, src, label, RESULT: items can be added without syntax error.
...
9 — What human maintainers must do (non-delegatable)
    ...
    Rotate keys & set secrets in the platform secret store.
    Make Supabase storage private and configure RLS and signed URLs.
...
11 — Next steps & checklist (actionable)
    ...
    Remove secrets from repo and rotate keys (HUMAN).
    Make Supabase bucket private and switch to signed URLs (HUMAN).
    Fix id bug and add listener cleanup for FurnitureItem and GridSlider (AUTOMATED OK, small cost).
